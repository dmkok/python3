...
text = input()
text2=text
text=re.split('\D', text)#\D убирает любые символы кроме цифр, соответственно знак минуса также убирается
sum=0
for i in text:
    if (len(i)!=0):
        i=int(i)
...
while i < l:
    s_int = ''
    a = text2[i]
    while '0' <= a <= '9':#проверяет что текущий символ входит в промежуток от 0 до 9 (знак - явно не будет учитываться)
        # и добавляет его во временную строку, для чисел > 9
        s_int+= a
        i+= 1
        if i < l:#проверка что строка не окончилась
            a = text2[i]
        else:
            break
    i+= 1
